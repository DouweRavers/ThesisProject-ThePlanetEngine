        ...
        void HandleMouseInput(Rect textureRect, Rect pointMenuRect)
        {
            ...
            if (e.type == EventType.MouseDown)
            {
                ...
                textureRect.y < e.mousePosition.y && e.mousePosition.y < textureRect.y + textureRect.height)
                {
                    ...
                    {
                        _planet.Data.OceanGradient.Points.Add(new GradientPoint(_color, positionNormalized, 1f));
    ...
    internal class BiomeGradientUI : ScriptableObject
    {
        ...
        Rect ShowPointProperties()
        {
            ...
            if (0 <= _selectedPoint)
            {
                ...
                GradientPoint prevPoint = new GradientPoint(point.Texture, point.Position, point.Weight);

                // Gradient point properties
                GUILayout.Label("Texture:");
                point.Texture = EditorGUILayout.ObjectField(_planet.Data.biomeGradient.Points[_selectedPoint].Texture, typeof(Texture2D), false) as Texture2D;
                point.UpdateColor();
                GUILayout.Label("Weight:");
                point.Weight = GUILayout.HorizontalSlider(point.Weight, 0, 5, GUILayout.Height(20));
                ...
                else
                {
                    ...
                    if (prevPoint.Texture != point.Texture ||
                        prevPoint.Position != point.Position ||
                        prevPoint.Weight != point.Weight)
                    {
                        GenerateTexture();
        ...
        void HandleMouseInput(Rect textureRect, Rect pointMenuRect)
        {
            ...
            if (e.type == EventType.MouseDown)
            {
                ...
                textureRect.y < e.mousePosition.y && e.mousePosition.y < textureRect.y + textureRect.height)
                {
                    ...
                    {
                        _planet.Data.biomeGradient.Points.Add(new GradientPoint(Texture2D.whiteTexture, positionNormalized, 1f));
