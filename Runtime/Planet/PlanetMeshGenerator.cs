...
public class PlanetMeshGenerator : MonoBehaviour
{
	...
	public void MakeQuad() {
		gameObject.GetComponent<MeshFilter>().mesh = GenerateUnitQuadMesh();
		Material material = new Material(defaultMaterial);
		gameObject.GetComponent<MeshRenderer>().material = material;
	}

	public void MakeCube() {
		gameObject.GetComponent<MeshFilter>().mesh = GenerateUnitCubeMesh();
		Material material = new Material(defaultMaterial);
		gameObject.GetComponent<MeshRenderer>().material = material;
	}
    ...
    Mesh GenerateUnitQuadMesh() { // basis for a plane
    ...
    Mesh GenerateMinimalUnitCubeMesh() {
        List<Vector3> newVertices = new List<Vector3>() {
        ...
        List<int> newIndexes = new List<int>() {
            2,0,1, 2,1,3, // FrontFace
			6,4,0, 6,0,2, // LeftFace
			3,1,5, 3,5,7, // RightFace
			0,4,5, 0,5,1, // TopFace
			3,7,6, 3,6,2, // BottomFace
			7,5,4, 7,4,6 // BackFace
		};
    ...
    Mesh SubdivideQuadMesh(Mesh mesh, int depth) {
		if (depth <= 0) {
			mesh.RecalculateNormals();
			mesh.RecalculateTangents();
